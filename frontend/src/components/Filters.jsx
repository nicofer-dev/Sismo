...
export default function Filters({ metadata, filters, setFilters }) {
  ...
  return <section className="filters-card">
    ...
    <div className="filters-grid">
      ...
      <label>Municipio<select value={filters.municipality} onChange={e => update("municipality", e.target.value)}><option value="">Todos</option>{municipalities.map(m => <option key={`${filters.department}-${m}`}>{m}</option>)}</select></label>
