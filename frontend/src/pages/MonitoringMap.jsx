...
export default function MonitoringMap({ metadata, data, filters, setFilters }) {
  ...
  const onEachFeature = (f, layer) => {
    ...
    const value = metricValue(item, metric);
    layer.bindTooltip(item ? `<strong>${item.municipio}</strong><br/>${item.departamento}<br/>${metricLabel}: ${metric === "criticidad" ? (item.criticidad || "Sin clasificación oficial") : fmt(value)}<br/>Daños: ${fmt(item.danos)} · Apoyo: ${fmt(item.apoyo)}<br/>Criticidad: ${item.criticidad || "Sin clasificación oficial"}` : `DIVIPOLA ${code}`, {sticky:true});
  ...
  return <div className="map-page"><section className="map-toolbar"><div><span className="eyebrow">MONITOREO MUNICIPAL</span><h1>Mapa de daños y afectaciones</h1><p>Los polígonos municipales se colorean según daños, apoyo o criticidad oficial.</p></div><label>Métrica del mapa<select value={metric} onChange={e => setMetric(e.target.value)}>{metadata?.map_metrics?.map(x => <option key={x.key} value={x.key}>{x.label}</option>)}</select></label></section>
    <Filters metadata={metadata} filters={filters} setFilters={setFilters}/><div className="map-note"><Info size={15}/><span>El color inicial representa daños y afectaciones. Los puntos de apoyo/acopio se muestran en una métrica separada; la criticidad oficial también puede consultarse sin mezclar conceptos.</span></div><div className="map-layout"><section className="map-card">{geo ? <MapContainer center={[4.3,-73.4]} zoom={5.3} minZoom={4} style={{height:"100%",width:"100%"}} zoomControl={true}><TileLayer attribution='&copy; OpenStreetMap contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/><GeoJSON key={`${metric}-${filters.department}-${filters.municipality}-${filters.category}-${data?.items?.length || 0}`} data={geo} style={style} onEachFeature={onEachFeature}/></MapContainer> : <div className="loading">Cargando cartografía municipal…</div>}<div className="legend"><strong>{metric === "criticidad" ? "Criticidad oficial" : metric === "danos" ? "Daños y afectaciones" : metric === "apoyo" ? "Apoyo y respuesta" : "Intensidad"}</strong><span><i style={{background:"#991B1B"}}/>{metric === "criticidad" ? "Crítica" : "Muy alta"}</span><span><i style={{background:"#DC2626"}}/>{metric === "criticidad" ? "Muy alta / alta" : "Alta"}</span><span><i style={{background:"#F97316"}}/>{metric === "criticidad" ? "Media-alta" : "Media-alta"}</span><span><i style={{background:"#FACC15"}}/>{metric === "criticidad" ? "Media" : "Media"}</span><span><i style={{background:"#BBF7D0"}}/>{metric === "criticidad" ? "Sin clasificación" : "Baja"}</span><span><i style={{background:"#E2E8F0"}}/>Sin registro</span></div></section>
      <aside className="detail-panel">{selected ? <><div className="detail-title"><MapPinned size={20}/><div><h2>{selected.municipio}</h2><span>{selected.departamento}</span></div></div><div className="detail-metric"><span>{metricLabel}</span><strong>{metric === "criticidad" ? (selected.criticidad || "Sin clasificación oficial") : fmt(metricValue(selected,metric))}</strong></div><dl><div><dt>Puntos totales</dt><dd>{fmt(selected.puntos)}</dd></div><div><dt>Daños y afectaciones</dt><dd>{fmt(selected.danos)}</dd></div><div><dt>Apoyo y respuesta</dt><dd>{fmt(selected.apoyo)}</dd></div><div><dt>Personas afectadas</dt><dd>{fmt(selected.afectados_personas)}</dd></div><div><dt>Heridos</dt><dd>{fmt(selected.heridos)}</dd></div><div><dt>Fallecidos</dt><dd>{fmt(selected.fallecidos)}</dd></div><div><dt>Criticidad oficial</dt><dd>{selected.criticidad || "Sin clasificación oficial"}</dd></div></dl><p className="detail-description">{selected.descripcion || "Sin descripción registrada."}</p></> : <div className="detail-empty"><MapPinned size={34}/><h2>Detalle municipal</h2>{data?.items?.length ? <><p>Selecciona un municipio del filtro actual:</p><div className="filtered-municipalities">{data.items.map(item => <button key={item.divipola} type="button" onClick={() => setSelected(item)}>{item.municipio}<span>{item.departamento}</span></button>)}</div></> : <p>No hay municipios para el filtro actual.</p>}</div>}</aside>
    </div></div>
}
